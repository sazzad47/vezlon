...
import CustomModelNode from './ModelNode'

const nodeTypes = { custom: CustomNode, CustomImgH, CustomModelNode };
const ImageH = () => {
  ...
  //     setFiles((prevFiles) => [...prevFiles, URL.createObjectURL(e.target.files)]);
...

{
  ...
  const onDrop = useCallback(
    (event) => {
      ...
      const newNode = {
        id: getId(),
        type: ((type ==='Load Model') || (type === 'Load Image'))? 'CustomImgH' :'custom',
        position,
        data: { label:`${type}`,
        setNodes,
        nodes,  },
    ...
    <ReactFlowProvider>
      <div className="reactflow-wrapper" ref={reactFlowWrapper}>
        ...
        >
          ...
          <Controls />
          {/* {files.map((file, i) => (
             <img key={i} src={file.url} height={100} width={100} />
          ))} */}
